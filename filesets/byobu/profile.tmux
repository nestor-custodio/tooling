...
#
set -g status-left ' #[fg=white,bold]#( hostname --short | tr "[:lower:]" "[:upper:]" )#( session="$( tmux display-message -p "#S" | tr "[:lower:]" "[:upper:]" )"; case "$session" in (.*|-|DEFAULT|[0-9]*) noop ;; (*) echo " (#[fg=orange]${session}#[default])" ;; esac )#[default] #(byobu-status tmux_left) '
